    ...
    for predictor in predictors:
        ...
        for trace in tracelist:
            instruction = (str(CHAMPSIM_EXE_PATH) + "/champsim_" + predictor,
                        ...
                        "--json", str(LOG_PATH) + "/("  + predictor[:predictor.find("_size")] + ")" + predictor[predictor.find("_size"):] + trace + ".json",
...
def merge_json(predictors):
    ...
    for predictor in size_independent_predictors:
        ...
        data = []
        for tracer_test in range(0,len(merge_list)): 
            ...
            with open(str(LOG_PATH)+ "/" + merge_list[tracer_test],"r") as fromfile:
                    ...
                    fromfile.close() # close the test file 
        ...
        with open(str(LOG_PATH)+ "/"+predictor + ".json","w+") as tofile:
            #len(json.load(tofile)) > len(predictors)
            tofile.seek(0)
