...
def main():
    global recompile, recompile_list
...
def config_setup(args: Namespace) -> None:
    ...
    global warmup_instructions, simulated_instructions, recompile, recompile_list, run_predictors
...
def run_instructions(tracelist: list[str], predictors: list[str]) -> None:
    ...
    for predictor in predictors:
        ...
        t = threading.Thread(target = create_test , args = [instruction_list,predictor])
        threads.append(t)
        t.start()
...
#     log_name = str(LOG_PATH) + predictor + "_log.txt"
#     for i in instruction_list:
#         with open(log_name,'w') as test_log:
#             procs = subprocess.run(i, shell=True, stdout=test_log)
#     for p in procs:
#         p.wait()
#     test_log.close()
def create_test(instruction_list,predictor):
    for i in instruction_list:
        thread = threading.Thread(target = run_command , args = [i,predictor])
        thread.start()

    for _ in instruction_list:
        thread.join()

def run_command(cmd,predictor):
    trace_file_path = cmd[-1]
    trace_filename = os.path.basename(trace_file_path)
    log_name = str(SPEED_PATH) + "/" + predictor + "_" + trace_filename +  "_log.txt"
    with open(log_name,'w') as test_log:
        p = subprocess.run(cmd, shell=False, stdout=test_log)
        test_log.close()
