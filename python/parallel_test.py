...
def main():
    ...
    # I know this is stupid but im too lazy to fix the sync issue
    input("wait for sync")

    merge_json(run_predictors)

    input("merge for sync")
...
def config_setup(args: Namespace) -> None:
    ...
    if args.recompile:
        if "all" in args.recompile:
            ...
            for predictor in args.predictors:
                if predictor in args.recompile:
...
def create_test(instruction_list):
    threads = []  # Create the list outside the loop to collect threads

    # Create and start a thread for each command in instruction_list
...
def run_command(cmd):
    with open(os.devnull, 'w') as ignore:
        subprocess.run(cmd, shell=False, stdout=ignore)
